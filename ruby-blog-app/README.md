...
> **Note**: This application uses Podman as the container engine. Podman uses the same `Dockerfile` and `docker-compose.yml` file formats, providing full compatibility with existing container workflows while offering enhanced security through rootless containers.

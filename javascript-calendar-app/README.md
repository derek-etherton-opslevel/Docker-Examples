...
- Podman Compose (version 1.0 or later) or use `podman compose` (built-in)
...
2. Start all services with Podman Compose:
   ...
   Or using built-in Podman compose:
   ```bash
   podman compose up -d
   ```
...
MongoDB data is persisted using a named volume (`mongodb_data`). The data persists even when containers are stopped or removed.
...
- If using Podman on macOS or Windows, ensure Podman Machine is running: `podman machine start`
- Some systems may require `podman-compose` to be installed separately: `pip install podman-compose`
- Podman uses rootless containers by default, which provides better security
- Volume paths may differ from Docker on some systems; refer to Podman documentation for your OS
